//
// Edited by MagicTZ on 2020/10/22.
//
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
  ...
  for (auto &kp : keypoints) {
    // remove the key points which are on the boundary of the image
    if (kp.pt.x < half_patch_size || kp.pt.x > image.cols - half_patch_size || kp.pt.y < half_patch_size ||
    ...
    for (int dx = -half_patch_size; dx < half_patch_size; dx++) {
      for (int dy = -half_patch_size; dy < half_patch_size; dy++) {
        uchar intensity = image.at<uchar>(kp.pt.y + dy, kp.pt.x + dx);
        m10 += dx * intensity;
        m01 += dy * intensity;
      }
    }
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
  for (auto &kp : keypoints) {
    // the angle of the moment and

    DescType d(256, false);
    for (int i = 0; i < 256; i++) {

      d[i] = 0;  // if kp goes outside, set d.clear()
      // Obtain the random points p,q
      float p_x = ORB_pattern[i * 4];
      float p_y = ORB_pattern[i * 4 + 1];
      float q_x = ORB_pattern[i * 4 + 2];
      float q_y = ORB_pattern[i * 4 + 3];

      // rotate the random points by angle theta
      float theta = kp.angle * pi / 180;  // 角度转弧度
      cv::Point2f p_theta(p_x * cos(theta) - p_y * sin(theta) + kp.pt.x, p_x * sin(theta) + p_y * cos(theta) + kp.pt.y);
      cv::Point2f q_theta(q_x * cos(theta) - q_y * sin(theta) + kp.pt.x, q_x * sin(theta) + q_y * cos(theta) + kp.pt.y);
      // remove the rotated points which are the out of the image
      if (p_theta.x < 0 || p_theta.x > image.cols || p_theta.y < 0 || p_theta.y > image.rows || q_theta.x < 0 ||
          q_theta.x > image.cols || q_theta.y < 0 || q_theta.y > image.rows) {
        d.clear();
        break;  //认为该点不适合做descriptor
      } else if (image.at<uchar>(p_theta.y, p_theta.x) <= image.at<uchar>(q_theta.y, q_theta.x)) {
        d[i] = 1;
      }
  ...
  int bad = 0;  // record the bad points of descriptors
  for (auto &d : desc) {
    if (d.empty()) bad++;
  }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
  ...
  auto start = chrono::system_clock::now();  // 添加计时器
  ...
  for (int i = 0; i < desc1.size(); ++i) {
    ...
    for (int j = 0; j < desc2.size(); ++j) {
      ...
      d = 0;
      for (int k = 0; k < 256; ++k) {
        if (desc1[i][k] != desc2[j][k]) d += 1;  //使用二维数组来获取数值
      }
      //            cout<<"d: "<<d<<endl;
  ...
  cout << "The brute match cost:" << time_used.count() << " s" << endl;
  for (auto &m : matches) {
